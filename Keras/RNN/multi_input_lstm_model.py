...
x_train = np.reshape(x_train, (50, 4, 2))   # LSTM에서 사용을 위한 형 변환 중요
...
# LSTM 신경망
model = Sequential()
model.add(LSTM(128, batch_input_shape=(1, 4, 2), stateful=True))    # 속성 2개
...
for i in range(pred_count):
    ...
    sample_in = np.reshape(sample_in, (1, 4, 2))  # 샘플 수, 타입 스탭, 속성 수
