...
class MainWindow(QMainWindow):
    ...
    def Action(self, row):  # 동작 과정함수
        ...
        elif self.MainBox.item(row, 0).text() == '이미지크롤링':
            Activity.ImageCrawling(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 사이즈 변환':
            Activity.Resize(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4],
                                self.activity_list[self.activity_list.index(row) + 6],
                                self.activity_list[self.activity_list.index(row) + 8])
        elif self.MainBox.item(row, 0).text() == '이미지 사이즈 정사각형 변환':
            Activity.ImageSqure(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 회색조 변환':
            Activity.ConvertGray(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 이진화 변환':
            Activity.Binary(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 히스토그램 균일화(칼라)':
            Activity.EqulizeHistColor(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 히스토그램 균일화(회색조)':
            Activity.EqulizeHistGray(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '이미지 가우시안 블러':
            Activity.GausianBlur(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4],
                                self.activity_list[self.activity_list.index(row) + 6],
                                self.activity_list[self.activity_list.index(row) + 8],
                                self.activity_list[self.activity_list.index(row) + 10])
        elif self.MainBox.item(row, 0).text() == '이미지 미디안 블러':
            Activity.MedianBlur(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4],
                                self.activity_list[self.activity_list.index(row) + 6])
        elif self.MainBox.item(row, 0).text() == '이미지 양방향 블러':
            Activity.BilateralBlur(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4],
                                self.activity_list[self.activity_list.index(row) + 6],
                                self.activity_list[self.activity_list.index(row) + 8],
                                self.activity_list[self.activity_list.index(row) + 10])
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 행 전체 삭제':
            Activity.RowDropNa(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 열 전체 삭제':
            Activity.ColumnDropNa(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 사용자값 삽입':
            Activity.FillNa(self, self.activity_list[self.activity_list.index(row) + 2], # 서보인 추가
                                self.activity_list[self.activity_list.index(row) + 4])
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 액셀 확인':
            Activity.IsNull(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 직전값으로 삽입':
            Activity.ffill(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 직후값으로 삽입':
            Activity.ffill(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 평균값으로 삽입':
            Activity.mean(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 중앙값으로 삽입':
            Activity.median(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 결측치 최빈값으로 삽입':
            Activity.mode(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == 'Kor_Ocr':
            Activity.Kor_Ocr(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == 'Eng_Ocr':
            Activity.Eng_Ocr(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '막대 그래프 시각화':
            Activity.plot_bar(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '꺾은선 그래프 시각화':
            Activity.plot_line(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 불용어 처리':
            Activity.Filter_Content(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 형태소 분석':
            Activity.Morphs(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '텍스트 명사 추출':
            Activity.Extract_Word(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
        elif self.MainBox.item(row, 0).text() == '워드 클라우드 시각화':
            Activity.Word_Cloud(self, self.activity_list[self.activity_list.index(row) + 2]) # 서보인 추가
    ...
    def AddItem(self):
        if self.ActivityBox.currentItem().text(1) != '':  # 변수칸이 비었을 때(목록이 추가되는걸 막기위해 추가)
            ...
            self.selectFolder() # 서보인 추가 폴더 열기 다이얼로그 실행 테스트 중 
            self.selectFolders() # 서보인 추가 폴더 열기 다이얼로그 실행 테스트 중 
    ...
    def AppendTable(self, activity_name):
        ...
        if activity_name == '브라우저 열기':  # 기능의 이름이 브라우저 열기 일때
            ...
            self.AppendList(activity_name)
        elif activity_name == '이미지크롤링':
            self.AppendList(activity_name)
        elif activity_name == '딜레이':
            self.AppendList(activity_name)
        elif activity_name == '반복_For':
            self.AppendList(activity_name)
        elif activity_name == '이미지 사이즈 변환':
            self.AppendList(activity_name)
        elif activity_name == '이미지 사이즈 정사각형 변환':
            self.AppendList(activity_name)
        elif activity_name == '이미지 회색조 변환':
            self.AppendList(activity_name)
        elif activity_name == '이미지 이진화 변환':
            self.AppendList(activity_name)
        elif activity_name == '이미지 히스토그램 균일화(칼라)':
            self.AppendList(activity_name)
        elif activity_name == '이미지 히스토그램 균일화(회색조)':
            self.AppendList(activity_name)
        elif activity_name == '이미지 가우시안 블러':
            self.AppendList(activity_name)
        elif activity_name == '이미지 미디안 블러':
            self.AppendList(activity_name)
        elif activity_name == '이미지 양방향 블러':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 행 전체 삭제':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 열 전체 삭제':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 사용자값 삽입':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 액셀 확인':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 직전값으로 삽입':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 직후값으로 삽입':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 평균값으로 삽입':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 중앙값으로 삽입':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 결측치 최빈값으로 삽입':
            self.AppendList(activity_name)
        elif activity_name == 'Kor_Ocr':
            self.AppendList(activity_name)
        elif activity_name == 'Eng_Ocr':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 불용어 처리':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 형태소 분석':
            self.AppendList(activity_name)
        elif activity_name == '텍스트 명사 추출':
            self.AppendList(activity_name)
        elif activity_name == '막대 그래프 시각화':
            self.AppendList(activity_name)
        elif activity_name == '꺾은선 그래프 시각화':
            self.AppendList(activity_name)
        elif activity_name == '워드 클라우드 시각화':
            self.AppendList(activity_name)
    ...
    def selectFolder(self):
        if 'fordername' in self.MainBox.item(self.MainBox.currentRow(), 1).text():
            name = QFileDialog.getExistingDirectory()
            self.activity_list[self.activity_list.index(self.row_num) + 2] = name
            
        else : 
            pass

    # 폴더 여러개 선택하기 // !!! 오류 잠재적 오류 !!!
    def selectFolders(self):

        if 'Original_Path' in self.MainBox.item(self.MainBox.currentRow(), 1).text():
            original_path = QFileDialog.getExistingDirectory()
            self.activity_list[self.activity_list.index(self.row_num) + 2] = original_path
            save_path = QFileDialog.getExistingDirectory()
            self.activity_list[self.activity_list.index(self.row_num) + 4] = save_path
    ...
    # 0819 서보인 modefinder 구현중
    def modefinder(self,numbers):

        c = Counter(numbers) # counter  함수를 통해 딕셔너리 형태로 값을 반환해준다 , 즉 빈도수를 알려주는 딕셔너리가 생성된다
        order = c.most_common() # 빈도수를 내림차순으로 다시 정렬한다
        maximum = order[0][1] # 최빈값을 찾아내기 위한 기준값을 설정한다

        modes = [] # 초기화를 위해 선언
        for num in order: # 반복문을 통해 최빈값을 찾아낸다
            if num[1] == maximum:
                modes.append(num[0])
        return modes # 최빈값을 반환 
