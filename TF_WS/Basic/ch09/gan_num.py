# GAN 모델을 이용해 단순히 랜덤한 숫자를 생성하는 아닌 원하는 손글씨 숫자를 생성하는 모델을 만들어봅니다.
...
Y = tf.compat.v1.placeholder(tf.float32, [None, n_class])     # 10개의 값
...
def discriminator(inputs, labels, reuse=None):
    with tf.compat.v1.variable_scope('discriminator') as scope:
        ...
        if reuse:
            scope.reuse_variables()     # 이전에 사용한 변수 재사용

        inputs = tf.concat([inputs, labels], 1)

        hidden = tf.layers.dense(inputs, n_hidden, activation=tf.nn.relu)
        # 손실값 계산에 sigmoid_cross_entropy_with_logits 함수 사용하기 위해 활성화 함수 사용안함
...
loss_D = loss_D_real + loss_D_gene  # 이 값을 최소화하면 구분자(경찰)를 학습 시킬수 있다.
