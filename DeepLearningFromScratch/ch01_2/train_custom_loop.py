...
for epoch in range(max_epoch):
    # 데이터 섞기
    idx = np.random.permutation(data_size)  # shuffle과 유사
    x = x[idx]
    t = t[idx]

    for iters in range(max_iters):
        batch_x = x[iters*batch_size:(iters+1)*batch_size]  # 데이터 배치
        batch_t = t[iters*batch_size:(iters+1)*batch_size]  # 정답 배치
