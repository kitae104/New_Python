...
print("샘플 크기 : \n", len(sentences))     # 61999
print("0번 째 샘플 단어 시퀀스 : \n", sentences[0])   # ['가락지빵', '주문', '하', '고', '싶', '어요']
print("0번 째 샘플 bio 태그 : \n", tags[0])           # ['B_FOOD', 'O', 'O', 'O', 'O', 'O']
print("샘플 단어 시퀀스 최대 길이 :", max(len(l) for l in sentences))              # 168
print("샘플 단어 시퀀스 평균 길이 :", (sum(map(len, sentences))/len(sentences)))   # 8.796238649010467
...
print("단어 사전 크기 :", vocab_size)             # 17752
...
print("학습 샘플 시퀀스 형상 : ", x_train.shape)     # (49599, 40)
print("학습 샘플 레이블 형상 : ", y_train.shape)     # (49599, 40, 10)
print("테스트 샘플 시퀀스 형상 : ", x_test.shape)    # (12400, 40)
print("테스트 샘플 레이블 형상 : ", y_test.shape)    # (12400, 40, 10)
...
def sequences_to_tag(sequences):
    ...
    for sequence in sequences:      # 전체 시퀀스로부터 시퀀스를 하나씩 꺼낸다.
        ...
        for pred in sequence:       # 시퀀스로부터 예측값을 하나씩 꺼낸다.
            ...
            temp.append(index_to_ner[pred_index].replace("PAD", "0"))   # 'PAD'는 'O'로 변경
        result.append(temp)
    return result
...
# F1 평가 결과
print(classification_report(test_tags, pred_tags))  # 정밀도, 재현율, f1
